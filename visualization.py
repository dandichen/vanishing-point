...
def point_visualization(x1, y1, x2, y2, img):
    img_show = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    # cv2.circle(np.int8(img_show), (np.int8(x), np.int8(y)), 200, (35, 0, 255), -1)
    fig = plt.figure()
    plt.imshow(img_show)
    plt.scatter([x1], [y1], s=100)          # blue dot, flowstereo(mxnet) detected vanishing point
    plt.scatter([x2], [y2], c='r', s=100)   # red dot, opencv dense detected vanishing point
    # plt.scatter([x3], [y3], c='g', s=100)   # green dot, opencv sparse detected vanishing point
