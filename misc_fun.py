...
def mxnet_flow(img0, sparse_corner_num, num):
    ...
    # flowstereo(mxnet)
    flow_path = os.path.join('/mnt/scratch/DandiChen/road/flow/12/flowstereo/value', '{:06d}'.format(num))
...
def dense_flow(img0, sparse_corner_num, num):
    num_superpixel, labels, _ = denseflow.get_superpixel(img0, num_superpixel=sparse_corner_num)

    # opencv dense flow
    flow_path = os.path.join('/mnt/scratch/DandiChen/road/flow/12/opencv_dense/value', '{:06d}'.format(num))
...
def sparse_flow(img0, sparse_corner_num, num):
    num_superpixel, labels, _ = denseflow.get_superpixel(img0, num_superpixel=sparse_corner_num)

    # opencv sparse flow
    flow_path = os.path.join('/mnt/scratch/DandiChen/road/flow/3/opencv_sparse/value', '{:06d}'.format(num))
